...
struct ws_vulkan_interface hybris_vulkan_interface = {
	...
	vulkan_helper_has_mapping,
	vulkan_helper_get_mapping,
};
...
/*
VULKAN_IDLOAD(vkAllocationFunction);
VULKAN_IDLOAD(vkReallocationFunction);
VULKAN_IDLOAD(vkFreeFunction);
VULKAN_IDLOAD(vkInternalAllocationNotification);
VULKAN_IDLOAD(vkInternalFreeNotification);
VULKAN_IDLOAD(vkVoidFunction);
*/
...
{
	return ws_vkWaitForFences(device, fenceCount, pFences, waitAll, timeout);
}

VkResult vkQueuePresentKHR(VkQueue queue, const VkPresentInfoKHR* pPresentInfo)
{
	return ws_vkQueuePresentKHR(queue, pPresentInfo);
}
